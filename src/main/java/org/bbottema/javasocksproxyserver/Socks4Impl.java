...
public class Socks4Impl {
    ...
    public InetAddress resolveExternalLocalIP() {
        ...
        if (m_ExtLocalIP != null) {
            Socket sct;
            try {
                sct = m_Parent.m_RemoteSocketFactory.createSocket(m_ExtLocalIP, m_Parent.getPort());
        ...
        for (String host : hosts) {
            try (Socket sct = m_Parent.m_RemoteSocketFactory.createSocket(InetAddress.getByName(host), 80)) {
                IP = sct.getLocalAddress();
                break;
            } catch (Exception e) {
